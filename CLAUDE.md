...
- Build the matched GNSS NPZ dataset: `python scripts/build_gnss_event_npz.py`
- Run experiment sweeps/searches: `python scripts/hyperparam_search.py` or `python scripts/sweep_baseline_params.py`
...
- Several scripts are one-off experiment utilities with hardcoded assumptions or local paths. Check them before reuse, especially `scripts/build_gnss_event_npz.py`, `scripts/final_eval.py`, and `src/evaluation/evaluate.py`.
