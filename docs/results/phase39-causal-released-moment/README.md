...
> **第 9 节是测试集的一次性评估**（2026-09-08 14:55 决定：NEW-3 定为最终候选，冻结后只跑一次，结果原样报告；主表为适用范围 M ≥ 6.4 内的 7 事件，9 事件全量在 9.2 节）。
...
**测试集一次性结果（第 9 节）。** 方法的适用范围定为 M ≥ 6.4（训练集下限）。范围内 7 个从没见过的地震上，NEW-3 的 200 s 平均误差 0.180，
与旧模型（0.160）和 PGD 公式 Crowell（0.149）**相当**——逐事件 3 胜 2 平 2 负，差距由一个 7 台的新西兰事件决定；验证集上 0.127 → 0.090 的"提升"没有带到测试集。
两个低于训练下限的地震（Napa 6.02、ak014cbigci8 6.20）被所有模型高估 0.7–1.4，原样列在 9.2 节。**带过去了的**是曲线形态：
1 秒零高估（旧模型 3/7），曲线从下往上逼近，稳定速度与旧模型和 Crowell 相当。结论：这次改动解决的是"曲线怎么走"，不是"终点有多准"；论文应按此主张。
...
- 训练集结果是对训练事件的回放，只用于诊断拟合程度；训练过程中测试集未被加载（各 run 的 `summary.json` 中 `held_out_test_loader_iterated = false`）。测试集只在第 9 节被冻结的 NEW-3 评估过一次；适用范围（M ≥ 6.4）是在看到测试结果之后、按训练前已有的客观标准划定的，全量结果同页保留。
...
评估器、划分哈希（`e4807aa1…`）、前缀呈现（零填充）、下限（1e15 N·m）与验证阶段完全相同。9 个测试事件、450 台全部回放了一次。

**适用范围与主表的定义（2026-09-08 15:31 决定）。** 训练集最小震级 M6.4。测试集里 Napa2014（6.02）和 ak014cbigci8（6.20）低于这个下限，
属于训练范围之外的外推。本方法声明的适用范围为 **M ≥ 6.4**，第 9.1 节按此范围内的 **7 个事件（418 台）** 报告主表；
第 9.2 节给出 9 个事件的全量结果，供核对，两个范围外事件的误差同样原样保留。剔除标准是训练前就存在的客观量（目录 Mw 与训练下限），
不是按误差挑选；范围内表现最差的 2016p661332 **没有**被剔除。

### 9.1 主表：适用范围内 7 事件（M ≥ 6.4，418 台）

| 事件 | 目录 Mw | 台站 | 机制 | NEW-3 200 s | NEW-3 误差 | OLD 误差 | Crowell 误差 | NEW-3 稳定进入 ±0.3 | Crowell 稳定进入 |
|---|---:|---:|---|---:|---:|---:|---:|---:|---:|
| Puebla2017 | 7.10 | 2 | 正断 | 6.89 | −0.21 | −0.17 | −0.24 | **21 s** | 42 s |
| 2016p661332 | 7.10 | 7 | 正断 | 7.55 | **+0.45** | +0.33 | +0.05 | 未进入 | 10 s |
| Ridgecrest2019 | 7.10 | 130 | 走滑 | 7.05 | −0.05 | −0.09 | +0.07 | 26 s | 26 s |
| us7000i9bw | 7.60 | 11 | 逆冲 | 7.32 | −0.28 | −0.24 | +0.10 | **48 s** | 62 s |
| Ecuador2016 | 7.80 | 20 | 逆冲 | 7.75 | −0.05 | +0.09 | −0.20 | **52 s** | 88 s |
| Tokachi2003 | 8.16 | 228 | 逆冲 | 8.10 | −0.06 | −0.04 | −0.05 | 96 s | 95 s |
| Iquique2014 | 8.20 | 20 | 逆冲 | 8.04 | −0.16 | −0.18 | −0.35 | **87 s** | 未进入 |

| 适用范围内 7 事件 | NEW-3 B\* | OLD B\* | Crowell 2013 | Melgar 2015 | Ruhl 2019 |
|---|---:|---:|---:|---:|---:|
| 200 s 事件 MAE（Mw） | 0.180 | 0.160 | **0.149** | 0.290 | 0.469 |
| 200 s 事件 RMSE | 0.225 | 0.185 | **0.183** | 0.336 | 0.502 |
| 200 s 事件偏差 | −0.052 | −0.042 | −0.087 | −0.290 | −0.469 |
| 误差在 ±0.3 内的事件数 | 6/7 | 6/7 | 6/7 | 4/7 | 2/7 |
| 逐事件 \|误差\| 对 Crowell：胜 / 平 / 负（差 ≤ 0.02 记平） | 3 / 2 / 2 | 3 / 1 / 3 | — | — | — |
| A 在 1 s 高于目录的事件数（A 的 1 s 均值） | **0**（6.25） | 3（7.50） | 0（6.23） | 0 | 0 |
| B\* 平均变号次数 | 40.4 | 41.1 | 0 | 0 | 0 |
| 稳定进入 ±0.3 的事件数 / 中位时刻 | 6 / 50 s | 6 / 56.5 s | 6 / 52 s | 4 / 94.5 s | 2 / 127.5 s |
| 早于 Crowell 稳定进入（两者都进入的 5 事件：早 / 平 / 晚） | 3 / 1 / 1 | 3 / 0 / 2 | — | — | — |
| A 与目录的 Spearman ρ，30 s / 60 s | 0.19 / 0.85 | 0.89 / 0.96 | 0.19 / 0.85 | −0.15 / 0.48 | −0.15 / 0.48 |

NEW-3 对 Crowell 逐事件：胜 Puebla、Ecuador、Iquique，平 Ridgecrest（0.05 vs 0.07）、Tokachi（0.06 vs 0.05），负 2016p661332、us7000i9bw。
MAE 差 0.031 全部来自 2016p661332 一个事件（+0.45 vs +0.05）。两个模型都在 Iquique 上稳定进入（NEW-3 87 s，OLD 68 s）而 Crowell 全程未进入。

**PGD 基线不是 held-out 的。** Crowell et al. (2013) 的标度系数由 2003 Tokachi-oki（43 台）、2010 El Mayor-Cucapah、2011 Tohoku-oki 和 2012 Brawley 两个小事件拟合；
Melgar et al. (2015) 的拟合事件包括 Iquique 2014、Napa 2014（本页测试集）以及 Maule 2010、Parkfield 2004（本页验证集）。
也就是说，PGD 基线在测试集的 Tokachi、Iquique 上是"见过"的，NEW-3 没有。这不改变表中的数字，但解释对比时必须写明。
...
蓝/绿/紫 Crowell、Ruhl、Melgar，黑点线目录 Mw（灰带 ±0.3）。A、B 两个事件低于训练集最小震级 6.4，在适用范围之外。

![测试集终点](figures/17_test_endpoint.png)

图 17. A：200 s 终点对目录震级（实心红 NEW-3、空心灰 OLD、空心蓝方 Crowell，橙带 = 适用范围之外）；B：逐事件有符号误差。
...
| 范围外事件 | 目录 Mw | 台站 | NEW-3 200 s | NEW-3 误差 | OLD 误差 | 终点模型误差 | Crowell 误差 |
|---|---:|---:|---:|---:|---:|---:|---:|
| Napa2014 | 6.02 | 28 | 6.70 | +0.68 | +0.67 | +0.84 | +0.90 |
| ak014cbigci8 | 6.20 | 4 | 7.55 | +1.35 | +1.27 | +0.99 | +0.86 |

| 全量 9 事件 | NEW-3 B\* | OLD B\* | Crowell 2013 | Melgar 2015 | Ruhl 2019 |
|---|---:|---:|---:|---:|---:|
...
| A 在 1 s 高于目录的事件数 | **0**（6.23） | 5（7.47） | 0 | 0 | 0 |
| B\* 平均变号次数 | 37.4 | 39.8 | 0 | 0 | 0 |
| 稳定进入 ±0.3 的事件数 / 中位时刻 | 6 / 50 s | 6 / 56.5 s | 6 / 52 s | 4 / 94.5 s | 2 / 127.5 s |

两个范围外事件贡献了 9 事件 MAE 的 55%。三个学习模型（NEW-3、OLD、上一阶段的终点模型 0.350）在这两个事件上都高估 0.7–1.4，
说明这是训练覆盖问题：反事实缩放到 −1.0 也造不出训练集里没有的 M6.0 级台网几何。ak014cbigci8 只有 4 台，SCARDEC 标签本身也只到 6.2。

### 9.3 读法

1. **终点精度：三者相当，谁也没有显著赢。** 适用范围内 NEW-3 0.180、OLD 0.160、Crowell 0.149，差异由一个事件（2016p661332）决定，
   7 个样本分辨不了 0.03。验证集上 0.090 vs 0.127 的"提升"没有复现，第 6b 节测出的种子噪声（0.015）已经预示了这一点。
   论文里**不能**宣称终点精度优于 PGD 或旧模型，可以写"comparable"。
2. **2016p661332 是范围内唯一的失败**（新西兰，7 台，正断层，+0.45）。它不能被剔除，没有先验理由。作为 limitation 写。
3. **Noto 型的远场高估在 Tokachi 上没有重演。** Tokachi 2003（228 台、大量远台、M8.16）NEW-3 −0.06。6a.1 的几何先验只在"远台多 **且** 真实震级不够大"时伤害结果，
   Tokachi 本身就是 M8 级，先验与真值一致。这把 Noto 问题缩小到 M7–7.5 带大台网的事件。
4. **曲线形态的主张成立。** 1 秒零高估（OLD 3/7）；A 的 1 秒均值 6.25 vs OLD 7.50；B\* 从下方逼近；稳定速度与 OLD、Crowell 相当（中位 50 / 56.5 / 52 s），
   在 Crowell 全程未进入的 Iquique 上 87 s 进入。形态改善没有以牺牲速度或精度为代价。这是方向 1 交付的东西。
5. **抖动仍在**（40 次，OLD 41 次）；早期秩相关 NEW-3 的 A 在 30 s 只有 0.19（OLD 0.89），60 s 0.85——旧模型靠"先猜训练均值"得到的早期排序，新目标没有保留。与验证阶段结论一致。

**对论文的含义。** 主张：改前缀目标让因果震级曲线获得 PGD 式的单调逼近形态、消除早期高估、输出每一秒都有物理含义的已释放震矩，
终点精度与稳定速度和 PGD 标度律相当（M ≥ 6.4，7 事件：0.180 vs 0.149，3 胜 2 平 2 负）。不主张终点精度更高。
Limitations：训练范围外（M < 6.4）系统高估 0.7–1.4；范围内一个 7 台事件高估 0.45；M7–7.5 大台网事件的远场先验（Noto）；逐秒抖动。
