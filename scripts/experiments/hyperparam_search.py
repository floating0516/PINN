...
def apply_config(config, params):
    ...
    stf_cfg["lambda_synth"] = params["lambda_synth"]
    stf_cfg["lambda_mag"] = params["lambda_mag"]
...
def run_single_config(name, params, config_template):
    ...
    print(f"\n{'='*80}")
    print(f"配置 {name}")
    print(f"  lr={params['lr']}, λ_wave={params['lambda_synth']}, "
          f"λ_physics={params['lambda_mag']}, λ_shape={params.get('lambda_shape', 0.1)}")
    print(f"{'='*80}")
...
def main():
    ...
    # =====================================================================
    # 超参数配置列表 (Round 2: L_synth 修复后的 λ_wave 消融实验)
    # 固定: lr=1e-4, λ_physics=0.2, λ_shape=0.1
    # 变量: λ_wave ∈ {0.0, 0.01, 0.02, 0.05, 0.1, 0.2}
    # =====================================================================
    configs = [
        ("B1", {"lr": 1e-4, "lambda_synth": 0.0,  "lambda_mag": 0.2, "lambda_shape": 0.1}),
        ("B2", {"lr": 1e-4, "lambda_synth": 0.01, "lambda_mag": 0.2, "lambda_shape": 0.1}),
        ("B3", {"lr": 1e-4, "lambda_synth": 0.02, "lambda_mag": 0.2, "lambda_shape": 0.1}),
        ("B4", {"lr": 1e-4, "lambda_synth": 0.05, "lambda_mag": 0.2, "lambda_shape": 0.1}),
        ("B5", {"lr": 1e-4, "lambda_synth": 0.1,  "lambda_mag": 0.2, "lambda_shape": 0.1}),
        ("B6", {"lr": 1e-4, "lambda_synth": 0.2,  "lambda_mag": 0.2, "lambda_shape": 0.1}),
    ]
    ...
    for r in results_sorted:
        ...
        print(f"{r['name']:<6} {p.get('lr', 0):>8.0e} {p.get('lambda_synth', 0):>8.3f} "
              f"{p.get('lambda_mag', 0):>8.3f} {p.get('lambda_shape', 0):>8.3f} "
    ...
    with open(RESULTS_FILE, "w", encoding="utf-8") as f:
        ...
        for r in results_sorted:
            ...
            f.write(f"{r['name']:<6} {p.get('lr', 0):>8.0e} {p.get('lambda_synth', 0):>8.3f} "
                    f"{p.get('lambda_mag', 0):>8.3f} {p.get('lambda_shape', 0):>8.3f} "
                    f"{r.get('best_val_loss', float('inf')):>10.4f} "
                    f"{r.get('test_mae', float('inf')):>10.4f} "
