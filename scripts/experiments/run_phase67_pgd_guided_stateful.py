...
SEQUENCE_START_SEC = 30
STEP_HUBER_BETA_MW = 0.01
HISTORY_HUBER_BETA_LOG10 = 0.05
TREND_OFFSETS = (5, 20, 60)
TREND_ALLOWANCES_MW = (0.02, 0.05, 0.10)
...
LOSS_WEIGHTS = {
    ...
    "pgd_advantage": 0.05,
    "tolerant_window_trend": 1.0,
    "plateau_oscillation": 1.0,
    "confirmed_history": 1.0,
    "confidence_supervision": 0.5,
}
NORMALIZER_FLOORS = {
    "endpoint_science": 1.0e-8,
    "progressive_sequence": 1.0e-8,
    "endpoint_teacher": 1.0e-4,
    "pgd_advantage": 1.0e-8,
    "tolerant_window_trend": 1.0e-3,
    "plateau_oscillation": 1.0e-3,
    "confirmed_history": 1.0e-8,
    "confidence_supervision": 1.0e-8,
}
PGD_EVENT_MAE_BASELINE = {
    30: 0.68788152440389,
    60: 0.44918486976623523,
    90: 0.3941603412628174,
    120: 0.3281883595784505,
    200: 0.26750747680664066,
}
VALIDATION_GATES = {
    "endpoint_event_mae_max": PHASE40_GATES["endpoint_event_mae_max"],
    "endpoint_station_mae_max": PHASE40_GATES["endpoint_station_mae_max"],
    "pgd_event_mae_improvement_min_mw": 0.01,
    "event_start_to_end_increase_fraction_min": 0.90,
    "event_20s_not_below_minus_0_05_fraction_min": 0.98,
    "event_post120_abs_step_p95_mw_max": 0.020,
    "event_post120_abs_step_max_mw_max": 0.150,
    "event_post120_excess_variation_p95_mw_max": 0.150,
    "event_post120_sign_changes_p95_max": 2.0,
    "event_peak_to_final_p95_mw_max": 0.150,
    "event_post160_band_width_p95_mw_max": 0.300,
}
...
def phase67_config() -> dict[str, Any]:
    ...
    config["model"]["stateful_streaming"] = {
        ...
        "shape_identity_start_sec": SHAPE_IDENTITY_START_SEC,
...
def _transition_parameters(model: PINNModel) -> list[torch.nn.Parameter]:
    ...
    if count != EXPECTED_TRANSITION_PARAMETER_COUNT:
        raise ValueError(
            f"Phase67 transition parameter count changed: {count} != "
...
def load_phase67_model(
    ...
    *,
    ...
    if incompatible.unexpected_keys:
        raise ValueError(
            "Phase39 source produced unexpected Phase67 keys: "
    ...
    if not incompatible.missing_keys or any(
        ...
        raise ValueError("Phase67 source load missing-key contract changed")
    total = sum(parameter.numel() for parameter in model.parameters())
    if total != EXPECTED_TOTAL_PARAMETER_COUNT:
        raise ValueError(
            f"Phase67 parameter count changed: {total} != "
...
def _assert_backbone_unchanged(
    ...
    if set(frozen_source) - set(current):
        raise ValueError("Phase67 checkpoint lost Phase39 backbone tensors")
    changed = [
        name
    ...
    if changed:
        raise RuntimeError(
            "Phase67 modified frozen Phase39 tensors: "
            + ", ".join(changed[:10])
        )
...
def stateful_target_rate(
    ...
    *,
    ...
    if config["model"]["stateful_streaming"]["mode"] != "pgd_residual_gru":
        raise ValueError("Phase67 requires pgd_residual_gru target semantics")
...
def _tensor_batch_with_hints(
    ...
    *,
    ...
    batch["pgd_mw"] = torch.as_tensor(
        np.asarray(hints.crowell_mw[indices]).copy(),
        device=device,
        dtype=torch.float32,
    )
    batch["pgd_valid"] = torch.as_tensor(
        np.asarray(hints.p_arrived[indices]).copy(),
        device=device,
        dtype=torch.bool,
    )
    return batch
...
def normalizer_training_indices(train_indices: np.ndarray) -> np.ndarray:
    ...
    if train_indices.ndim != 1 or len(train_indices) < sample_count:
        raise ValueError("Phase67 has insufficient training records for audit")
...
def stateful_loss_components(
    ...
    *,
    ...
    states, encoded_states, state_mw, gates, stream_diagnostics = (
    ...
    sequence_start = HORIZONS.index(SEQUENCE_START_SEC)
    encoded_error = (
        encoded_states[:, sequence_start:]
        - target_encoded[:, sequence_start:]
    ).pow(2).mean(dim=2)
    magnitude_error = torch.abs(
        state_mw[:, sequence_start:] - target_mw[:, sequence_start:]
    )
    ...
    target_confidence = torch.clamp(
        target_moment / full_moment.unsqueeze(1),
        min=0.0,
        max=1.0,
    )
    information_weight = 0.05 + 0.95 * target_confidence
    sequence_information = information_weight[:, sequence_start:]
    sequence_per_sample = torch.sum(
        sequence_information * (encoded_error + magnitude_error),
        dim=1,
    ) / sequence_information.sum(dim=1).clamp_min(1.0e-12)
    progressive_sequence = _weighted_mean(
        sequence_per_sample,
        sample_weights,
    )
    ...
    catalog = batch["magnitude_catalog"].reshape(-1, 1)
    state_catalog_error = torch.abs(state_mw - catalog)
    pgd_catalog_error = torch.abs(batch["pgd_mw"] - catalog)
    pgd_violation = torch.relu(
        state_catalog_error
        - pgd_catalog_error
        + PGD_ADVANTAGE_MARGIN_MW
    )
    pgd_loss = F.smooth_l1_loss(
        pgd_violation,
        torch.zeros_like(pgd_violation),
        reduction="none",
        beta=STEP_HUBER_BETA_MW,
    )
    pgd_valid = batch["pgd_valid"].to(dtype=state_mw.dtype)
    pgd_count = pgd_valid.sum(dim=1).clamp_min(1.0)
    pgd_per_sample = (pgd_loss * pgd_valid).sum(dim=1) / pgd_count
    pgd_advantage = _weighted_mean(pgd_per_sample, sample_weights)

    trend_per_sample = state_mw.new_zeros(state_mw.shape[0])
    trend_violation_mean = state_mw.new_zeros(())
    for offset, allowance in zip(TREND_OFFSETS, TREND_ALLOWANCES_MW):
        ...
        trend_per_sample = trend_per_sample + term
        trend_violation_mean = trend_violation_mean + violation.mean()
    trend_per_sample = trend_per_sample / len(TREND_OFFSETS)
    trend_violation_mean = trend_violation_mean / len(TREND_OFFSETS)
    tolerant_window_trend = _weighted_mean(
        trend_per_sample,
        sample_weights,
    )

    plateau_start = HORIZONS.index(PLATEAU_START_SEC)
    plateau_steps = torch.diff(state_mw[:, plateau_start:], dim=1)
    cancellation = (
        torch.abs(plateau_steps[:, :-1])
        + torch.abs(plateau_steps[:, 1:])
        - torch.abs(plateau_steps[:, :-1] + plateau_steps[:, 1:])
    )
    oscillation_violation = torch.relu(
        cancellation - OSCILLATION_ALLOWANCE_MW
    )
    oscillation_per_sample = F.smooth_l1_loss(
        oscillation_violation,
        torch.zeros_like(oscillation_violation),
        reduction="none",
        beta=STEP_HUBER_BETA_MW,
    ).mean(dim=1)
    plateau_oscillation = _weighted_mean(
    ...
    confidence_error = (
        stream_diagnostics["plateau_confidence"] - target_confidence
    ).pow(2).mean(dim=1)
    confidence_supervision = _weighted_mean(
        confidence_error,
        sample_weights,
    )
    ...
    components = {
        ...
        "pgd_advantage": pgd_advantage,
        "tolerant_window_trend": tolerant_window_trend,
        "plateau_oscillation": plateau_oscillation,
    ...
    diagnostics = {
        ...
        "mean_absorption_gate": float(
            stream_diagnostics["absorption_gate"].detach().mean().cpu()
        ),
        ...
        "endpoint_plateau_confidence": float(
            stream_diagnostics["plateau_confidence"][:, -1]
            .detach()
            .mean()
            .cpu()
        ),
        "mean_pgd_residual_mw": float(
            stream_diagnostics["pgd_residual_mw"].detach().mean().cpu()
        ),
        "mean_pgd_advantage_violation_mw": float(
            (pgd_violation * pgd_valid).detach().sum().cpu()
            / pgd_valid.detach().sum().clamp_min(1.0).cpu()
        ),
        "mean_trend_violation_mw": float(
            trend_violation_mean.detach().cpu()
        ),
...
def normalized_loss(
    ...
    if set(components) != set(LOSS_WEIGHTS):
        raise ValueError("Phase67 loss component set changed")
    if set(normalizers) != set(LOSS_WEIGHTS):
        raise ValueError("Phase67 normalizer set changed")
    ...
    if total is None:  # pragma: no cover
        raise AssertionError("Phase67 objective is empty")
    return total
...
def audit_loss_scales(
    *,
    ...
    config = phase67_config()
    validate_source_artifacts()
    configure_runtime(42, device)
    torch.manual_seed(42)
    model, frozen_source = load_phase67_model(config, device=device)
    ...
    for start in range(0, len(audit_indices), BATCH_SIZE):
        ...
        batch = _tensor_batch_with_hints(
            cache,
            hints,
            indices,
            device=device,
    ...
    if sample_count == 0 or first_batch is None:
        raise ValueError("Phase67 audit produced no samples")
    normalizers = {
    ...
    payload = {
        ...
        "hint_p_arrived_sha256": hints.manifest["outputs"]["p_arrived.npy"],
        ...
        "pgd_hint": "causal_crowell_mw_plus_p_arrived",
        "normalized_gradient_norms_first_batch": gradient_norms,
        "weighted_normalized_gradient_norms_first_batch": {
            name: float(LOSS_WEIGHTS[name]) * gradient_norms[name]
            for name in LOSS_WEIGHTS
        },
...
def load_normalizers(
    ...
    *,
    ...
    if payload.get("status") != "loss_scale_audit_complete":
        raise ValueError("Phase67 normalizer audit is incomplete")
    if payload["cache_raw_rates_sha256"] != cache.manifest["raw_rates_sha256"]:
        raise ValueError("Phase67 normalizers use a different raw cache")
    if payload["cache_arrays_sha256"] != cache.manifest["arrays_sha256"]:
        raise ValueError("Phase67 normalizers use different cache arrays")
    if payload["hint_crowell_mw_sha256"] != (
        hints.manifest["outputs"]["crowell_mw.npy"]
    ):
        raise ValueError("Phase67 normalizers use a different Crowell cache")
    if payload["hint_p_arrived_sha256"] != (
        hints.manifest["outputs"]["p_arrived.npy"]
    ):
        raise ValueError("Phase67 normalizers use different P-arrival hints")
    if payload["loss_weights"] != LOSS_WEIGHTS:
        raise ValueError("Phase67 normalizer loss weights changed")
    ...
    if not isinstance(sampling, dict) or sampling.get("method") != (
        ...
        raise ValueError("Phase67 normalizers use the biased audit sampling")
    normalizers = {
    ...
    if any(not math.isfinite(value) or value <= 0.0 for value in normalizers.values()):
        raise ValueError("Phase67 normalizers must be positive and finite")
    return normalizers
    ...
    *,
    ...
    with torch.no_grad():
        for batch_index, start in enumerate(
            ...
            states, _, state_mw, gates, stream_diagnostics = (
                model.stream_sequence_from_rates(
                    batch["raw_rate"],
                    horizons_sec=HORIZONS,
                    source_distance_m=batch["source_distance_m"],
            ...
            diagnostic_sums["absorption_gate"] += float(
                stream_diagnostics["absorption_gate"].sum().cpu()
            )
            diagnostic_sums["revision_mw"] += float(
                stream_diagnostics["revision_mw"].abs().sum().cpu()
            )
            diagnostic_sums["plateau_confidence"] += float(
                stream_diagnostics["plateau_confidence"].sum().cpu()
            )
    ...
    if not state_cubes:
        raise ValueError("Phase67 validation produced no batches")
    ...
    pgd_mw = np.asarray(hints.crowell_mw[selected])
    ...
    event_pgd_mw = _event_series(pgd_mw, events)
    event_array = np.asarray(events)
    event_names = sorted(set(events))
    event_catalogs = np.asarray(
        [
            np.median(catalogs[event_array == event])
            for event in event_names
        ]
    )
    key_event_mae: list[float] = []
    for horizon, pgd_baseline in PGD_EVENT_MAE_BASELINE.items():
        horizon_index = HORIZONS.index(horizon)
        event_mae = float(
            np.mean(np.abs(event_mw[:, horizon_index] - event_catalogs))
        )
        pgd_event_mae = float(
            np.mean(np.abs(event_pgd_mw[:, horizon_index] - event_catalogs))
        )
        metrics[f"event_mae_{horizon}s"] = event_mae
        metrics[f"pgd_event_mae_{horizon}s"] = pgd_event_mae
        metrics[f"pgd_event_mae_improvement_{horizon}s"] = (
            pgd_event_mae - event_mae
        )
        key_event_mae.append(event_mae)
        if max_batches is None and not math.isclose(
            pgd_event_mae,
            pgd_baseline,
            rel_tol=0.0,
            abs_tol=2.0e-6,
            ...
            raise ValueError(
                f"Phase67 frozen Crowell baseline changed at {horizon}s"
            )

    event_steps = np.diff(event_mw, axis=1)
    twenty_offset = 20
    twenty_delta = event_mw[:, twenty_offset:] - event_mw[:, :-twenty_offset]
    post120 = event_mw[:, np.asarray(HORIZONS) >= PLATEAU_START_SEC]
    post120_steps = np.diff(post120, axis=1)
    post120_total_variation = np.sum(np.abs(post120_steps), axis=1)
    post120_net = np.abs(post120[:, -1] - post120[:, 0])
    post120_excess = post120_total_variation - post120_net
    active_steps = np.where(
        np.abs(post120_steps) >= 0.005,
        np.sign(post120_steps),
        0.0,
    )
    sign_changes = []
    for row in active_steps:
        signs = row[row != 0.0]
        sign_changes.append(
            int(np.count_nonzero(signs[1:] != signs[:-1]))
            if signs.size > 1
            else 0
        )
    post160 = event_mw[:, np.asarray(HORIZONS) >= 160]
    metrics.update(
        {
            "mean_key_event_mae": float(np.mean(key_event_mae)),
            "event_start_to_end_increase_fraction": float(
                np.mean(event_mw[:, -1] >= event_mw[:, 0])
            ),
            "event_20s_not_below_minus_0_05_fraction": float(
                np.mean(twenty_delta >= -0.05)
            ),
            "event_post120_abs_step_p95_mw": float(
                np.percentile(np.abs(post120_steps), 95)
            ),
            "event_post120_abs_step_max_mw": float(
                np.max(np.abs(post120_steps))
            ),
            "event_post120_excess_variation_p95_mw": float(
                np.percentile(post120_excess, 95)
            ),
            "event_post120_sign_changes_p95": float(
                np.percentile(sign_changes, 95)
            ),
            "event_peak_to_final_p95_mw": float(
                np.percentile(
                    np.max(post120, axis=1) - post120[:, -1],
                    95,
                )
            ),
            "event_post160_band_width_p95_mw": float(
                np.percentile(
                    np.max(post160, axis=1) - np.min(post160, axis=1),
                    95,
                )
            ),
            "progressive_target_station_mw_mae": float(
                np.mean(target_errors[:, HORIZONS.index(SEQUENCE_START_SEC) :])
            ),
        }
    )
    diagnostics = {
        "validation_mean_retention_gate": gate_sum / max(gate_count, 1),
        **{
            f"validation_mean_{name}": value / max(diagnostic_count, 1)
...
def validation_gate(metrics: Mapping[str, Any]) -> dict[str, Any]:
    ...
    pgd_ratios = {
        f"pgd_improvement_{horizon}s_ratio": (
            VALIDATION_GATES["pgd_event_mae_improvement_min_mw"]
            / max(
                float(metrics[f"pgd_event_mae_improvement_{horizon}s"]),
                1.0e-12,
            )
        )
    ...
    ratios = {
        "endpoint_event_ratio": (
            float(metrics["endpoint_event_mae"])
            / VALIDATION_GATES["endpoint_event_mae_max"]
        ),
        "endpoint_station_ratio": (
            float(metrics["endpoint_station_mae"])
            / VALIDATION_GATES["endpoint_station_mae_max"]
        ),
        **pgd_ratios,
        "event_start_to_end_increase_ratio": (
            VALIDATION_GATES["event_start_to_end_increase_fraction_min"]
            / max(
                float(metrics["event_start_to_end_increase_fraction"]),
                1.0e-12,
            )
        ),
        "event_20s_trend_ratio": (
            VALIDATION_GATES[
                "event_20s_not_below_minus_0_05_fraction_min"
            ]
            / max(
                float(
                    metrics["event_20s_not_below_minus_0_05_fraction"]
                ),
                1.0e-12,
            )
        ),
        "event_post120_abs_step_p95_ratio": (
            float(metrics["event_post120_abs_step_p95_mw"])
            / VALIDATION_GATES["event_post120_abs_step_p95_mw_max"]
        ),
        "event_post120_abs_step_max_ratio": (
            float(metrics["event_post120_abs_step_max_mw"])
            / VALIDATION_GATES["event_post120_abs_step_max_mw_max"]
        ),
        "event_post120_excess_variation_ratio": (
            float(metrics["event_post120_excess_variation_p95_mw"])
            / VALIDATION_GATES[
                "event_post120_excess_variation_p95_mw_max"
            ]
        ),
        "event_post120_sign_changes_ratio": (
            float(metrics["event_post120_sign_changes_p95"])
            / VALIDATION_GATES["event_post120_sign_changes_p95_max"]
        ),
        "event_peak_to_final_ratio": (
            float(metrics["event_peak_to_final_p95_mw"])
            / VALIDATION_GATES["event_peak_to_final_p95_mw_max"]
        ),
        "event_post160_band_width_ratio": (
            float(metrics["event_post160_band_width_p95_mw"])
            / VALIDATION_GATES["event_post160_band_width_p95_mw_max"]
        ),
    }
    score = max(ratios.values())
    endpoint_preserved = endpoint_event_pass and endpoint_station_pass
    pgd_accuracy_passed = all(value <= 1.0 for value in pgd_ratios.values())
    trajectory_keys = set(ratios) - {
        "endpoint_event_ratio",
        "endpoint_station_ratio",
        *pgd_ratios,
    }
    trajectory_passed = all(ratios[key] <= 1.0 for key in trajectory_keys)
    return {
        ...
        "pgd_accuracy_passed": pgd_accuracy_passed,
        **ratios,
        "selection_score": score,
        "trajectory_passed": trajectory_passed,
        "passed": (
            endpoint_preserved and pgd_accuracy_passed and trajectory_passed
        ),
    }
...
def closest_candidate_rank(
    ...
    return (
        ...
        float(metrics["mean_key_event_mae"]),
        float(metrics["endpoint_event_mae"]),
        float(metrics["endpoint_station_mae"]),
    )
    ...
    *,
    ...
    return {
        ...
        "pgd_hint": {
            ...
            "valid_feature": "p_arrived",
            "role": "causal physical hint, not returned estimate",
        },
        "complete_forecast_target": {
            "schedule": "quadratic_ease_out",
            "sequence_loss_start_sec": SEQUENCE_START_SEC,
        ...
        "proposal_correction": {
            ...
            "identity_start_sec": SHAPE_IDENTITY_START_SEC,
        },
        "stateful_total_moment": {
            "coordinate": "crowell_mw_plus_recurrent_residual",
            "equation": (
        ...
        "trend_offsets": list(TREND_OFFSETS),
        "trend_allowances_mw": list(TREND_ALLOWANCES_MW),
        ...
        "cache_raw_rates_sha256": cache.manifest["raw_rates_sha256"],
        "cache_arrays_sha256": cache.manifest["arrays_sha256"],
        "hint_cache_root": str(hints.root),
        "hint_crowell_mw_sha256": hints.manifest["outputs"]["crowell_mw.npy"],
        "hint_p_arrived_sha256": hints.manifest["outputs"]["p_arrived.npy"],
        "pgd_event_mae_baseline": dict(PGD_EVENT_MAE_BASELINE),
        "validation_gates": dict(VALIDATION_GATES),
        "selection": (
            "require every endpoint, PGD-accuracy, trend, and plateau gate; "
            "then minimize mean five-horizon Event MAE, endpoint Event MAE, "
            "and endpoint Station MAE; never ensemble"
        ),
        "closest_checkpoint": (
            "diagnostic only: minimize worst normalized gate ratio, mean key "
            "Event MAE, endpoint Event MAE, and endpoint Station MAE"
        ),
        "hidden_data": (
    ...
    for epoch in range(1, epoch_count + 1):
        ...
        for batch_index, start in enumerate(range(0, len(shuffled), BATCH_SIZE)):
            ...
            if not bool(torch.isfinite(total)):
                raise FloatingPointError("Phase67 objective became non-finite")
            total.backward()
        ...
        if seen == 0:
            raise ValueError("Phase67 training produced no samples")
        _assert_backbone_unchanged(model, frozen_source)
        ...
        rows.append(row)
        if gate is not None and bool(gate["passed"]):
            rank = (
                float(validation_metrics["mean_key_event_mae"]),
                float(validation_metrics["endpoint_event_mae"]),
                float(validation_metrics["endpoint_station_mae"]),
            )
            if rank < best_rank:
                best_rank = rank
                best_epoch = epoch
        ...
        print(
            ...
            f"key={row['mean_key_event_mae']:.6f} "
            f"pgd60={row['pgd_event_mae_improvement_60s']:.6f} "
            f"step120={row['event_post120_abs_step_p95_mw']:.6f} "
            f"osc={row['event_post120_excess_variation_p95_mw']:.6f} "
    ...
    if smoke:
        ...
        selected = (
            min(
                passing,
                key=lambda summary: (
                    float(summary["selected_metrics"]["mean_key_event_mae"]),
                    float(summary["selected_metrics"]["endpoint_event_mae"]),
                    float(summary["selected_metrics"]["endpoint_station_mae"]),
                ),
            )
            if passing
            else None
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Finalized causal Crowell hint cache. This is explicit so a "
            "prototype or differently aligned cache cannot be selected "
            "silently."
        ),
    )
