...
def _record_arrays(
    ...
    *,
    ...
    if any(array.ndim != 1 for array in arrays) or len(
        ...
        raise ValueError(f"invalid raw PGD component shape: {record.event}/{record.station}")
    return arrays
...
def build_raw_pgd_records(
    ...
    factor = _unit_factor(config["dataset"]["units"])
    ...
    with np.load(config["paths"]["data_path"], allow_pickle=True) as data:
        for record in _iter_normalized_station_records(data):
            ...
            time_sec, east, north, up = _record_arrays(record, factor=factor)
...
def _baseline_value(values: np.ndarray, time_sec: np.ndarray, p_arrival_sec: float) -> float:
...
def causal_pgd_3d(
    ...
    *,
    ...
    if observation_horizon_sec not in SELECTED_HORIZONS:
        raise ValueError("unsupported PGD observation horizon")
    issue_time_sec = float(observation_horizon_sec) + float(processing_delay_sec)
    finite = (
    ...
    east = east - _baseline_value(east, time_sec, record.p_arrival_sec)
    north = north - _baseline_value(north, time_sec, record.p_arrival_sec)
    up = up - _baseline_value(up, time_sec, record.p_arrival_sec)
    observed = (time_sec >= 0.0) & (time_sec < float(observation_horizon_sec))
