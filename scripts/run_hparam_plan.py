...
LAMBDA_PHYSICS_VALUES = [0.3, 0.4, 0.5, 0.6]
...
def build_trial_config(base_config: dict[str, Any], lambda_physics: float, seed: int) -> dict[str, Any]:
    ...
    stf_rate_cfg['lambda_physics'] = float(lambda_physics)
    return config
...
def main() -> None:
    ...
    trial_plan = []
    for idx, lambda_physics in enumerate(LAMBDA_PHYSICS_VALUES, start=1):
        trial_plan.append(
            {
    ...
    search_space = {
        ...
        'parameter': 'training.stf_rate_loss.lambda_physics',
        'values': LAMBDA_PHYSICS_VALUES,
    ...
    for trial in trial_plan:
        ...
        lambda_physics = float(trial['lambda_physics'])
        random_seed = int(trial['random_seed'])
        print(f'[{trial_id}] 开始 | lambda_physics={lambda_physics}')

        trial_config = build_trial_config(base_config, lambda_physics=lambda_physics, seed=random_seed)
