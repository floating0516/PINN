...
策略概览 (7 阶段，逐步消融 + 联合精调):
  ...
  Phase 3  lambda_physics — 震级物理一致性消融
  Phase 4  lambda_shape   — 波形形状约束消融
  Phase 5  lambda_smooth / lambda_wave — 平滑 & 波动方程消融
...
def _build_phase_configs() -> List[Tuple[str, str, Dict[str, Any]]]:
    ...
    # ── Phase 5: lambda_smooth & lambda_wave 联合消融 ───────────────
    for sm in [0.0, 0.05, 0.1]:
        for wv in [0.0, 0.02, 0.05, 0.1]:
            if sm == 0.0 and wv == 0.0:
                continue  # 跳过全零（与 baseline 重复）
            cfgs.append((
                f"P5_sm{sm:.2f}_wv{wv:.2f}".replace(".", ""),
                "P5_smooth_wave",
                {"lambda_smooth": sm, "lambda_wave": wv},
            ))
...
def _build_phase6_configs(
    ...
    best_physics = best_per_phase.get("P3_physics", {}).get("lambda_physics", 0.2)
    best_shape = best_per_phase.get("P4_shape", {}).get("lambda_shape", 0.1)
    best_smooth = best_per_phase.get("P5_smooth_wave", {}).get("lambda_smooth", 0.0)
    best_wave = best_per_phase.get("P5_smooth_wave", {}).get("lambda_wave", 0.0)

    base_combo = {
        "lambda_nonneg": best_nonneg,
        "lambda_physics": best_physics,
        "lambda_shape": best_shape,
        "lambda_smooth": best_smooth,
        "lambda_wave": best_wave,
    }
...
def apply_config(
    ...
    stf_cfg["lambda_physics"] = float(merged_loss["lambda_physics"])
    stf_cfg["lambda_shape"] = float(merged_loss["lambda_shape"])
    stf_cfg["lambda_smooth"] = float(merged_loss["lambda_smooth"])
    stf_cfg["lambda_wave"] = float(merged_loss["lambda_wave"])
...
def update_figures(records: List[Dict[str, Any]], task_dir: Path) -> None:
    ...
    # ── 图 2: 各 lambda 散点图 ──────────────────────────────────────
    loss_keys = [
        "lambda_MSE", "lambda_nonneg", "lambda_physics",
        "lambda_shape", "lambda_smooth", "lambda_wave",
    ]
...
def _run_single_trial(
    ...
    record: Dict[str, Any] = {
        ...
        "lambda_physics": float(merged_loss["lambda_physics"]),
        "lambda_shape": float(merged_loss["lambda_shape"]),
        "lambda_smooth": float(merged_loss["lambda_smooth"]),
        "lambda_wave": float(merged_loss["lambda_wave"]),
    ...
    try:
        ...
        if datetime.now() < deadline:
            ...
            if global_best_rec is not None:
                best_loss_for_p7 = {
                    k: global_best_rec[k]
                    for k in ["lambda_MSE", "lambda_nonneg", "lambda_physics",
                              "lambda_shape", "lambda_smooth", "lambda_wave"]
                }
            else:
                best_loss_for_p7 = {}
    ...
    with open(final_report, "w", encoding="utf-8") as file:
        ...
        for item in valid_records[:10]:
            file.write(
                ...
                f"ph={item.get('lambda_physics',0):.2f} "
                f"sh={item.get('lambda_shape',0):.2f} "
                f"sm={item.get('lambda_smooth',0):.2f} "
                f"wv={item.get('lambda_wave',0):.2f}\n"
            )
