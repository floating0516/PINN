    ...
    config["model"]["stateful_streaming"].update(
        {
            ...
            "late_evidence_assimilation_anchor_start_sec": 160,
        }
    )
    return config
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "mode": "released_stf_gru",
                "use_late_evidence_assimilation_anchor": 1,
            },
            "boolean",
        ),
        (
            {
                "mode": "released_stf_gru",
                "use_late_evidence_assimilation_anchor": True,
                "late_evidence_assimilation_anchor_start_sec": 60,
            },
            "must be after",
        ),
    ],
...
def test_late_assimilation_anchor_must_start_before_source_end() -> None:
    config = _forecast_config()
    config["model"]["stateful_streaming"][
...
def test_late_assimilation_anchor_accepts_endpoint_evidence() -> None:
    model = PINNModel(_forecast_config()).eval()
    transition = model.released_stf_transition
    assert transition is not None
    low = torch.full((1, 200), 2.0e16)
    high = torch.full((1, 200), 2.0e18)
    with torch.no_grad():
        transition.moment_update_head.weight.zero_()
        transition.moment_update_head.bias.copy_(
    ...
    low_state = model.stream_step_from_rate(
        low,
        state=None,
        horizon_sec=199,
        source_distance_m=torch.zeros(1),
    ...
    upward = model.stream_step_from_rate(
        high,
        state=low_state.state,
        horizon_sec=200,
        source_distance_m=torch.zeros(1),
        source_dt_sec=torch.ones(1),
        beta_m_per_s=4_533.0,
    ...
    torch.testing.assert_close(
        torch.sum(upward.released_rate),
        torch.sum(high),
    )

    high_state = model.stream_step_from_rate(
        high,
        state=None,
        horizon_sec=199,
        source_distance_m=torch.zeros(1),
    ...
    downward = model.stream_step_from_rate(
        low,
        state=high_state.state,
        horizon_sec=200,
        source_distance_m=torch.zeros(1),
        source_dt_sec=torch.ones(1),
        beta_m_per_s=4_533.0,
    )
    expected_downward = torch.sum(high) - 0.03 * (
        torch.sum(high) - torch.sum(low)
    )
    torch.testing.assert_close(
        torch.sum(downward.released_rate),
        expected_downward,
    )
...
def test_rebase_window_uses_bounded_candidate_evidence() -> None:
    model = PINNModel(_stateful_config()).eval()
    transition = model.released_stf_transition
    assert transition is not None
    raw = torch.full((1, 200), 2.0e18)
    ...
    first_moment = torch.sum(first.released_rate)
    second_moment = torch.sum(second.released_rate)
    candidate_moment = torch.sum(raw * 0.01 * second.support_fraction)
    lower_bound = (
        1.0 - transition.early_moment_down_fraction_per_step
    ) * first_moment
    assert second_moment >= lower_bound
    assert candidate_moment < second_moment < first_moment


def test_rebase_window_caps_each_downward_step() -> None:
    model = PINNModel(_stateful_config()).eval()
    transition = model.released_stf_transition
    assert transition is not None
    source_distance = torch.zeros(1)
    ...
    max_drop = torch.max(magnitude[:-1] - magnitude[1:])
    theoretical_bound = (2.0 / 3.0) * -math.log10(
        1.0 - transition.early_moment_down_fraction_per_step
    )
    assert float(max_drop) <= theoretical_bound + 1.0e-5
...
def test_phase50_training_runner_contracts_are_enforced_together() -> None:
    ...
    passing = {
        ...
        "event_downward_step_max_mw": (
            VALIDATION_GATES["event_downward_step_max_mw_max"] * 0.9
        ),
    ...
    metric_to_gate = {
        ...
        "event_downward_step_max_mw": "event_downward_step_max_mw_max",
