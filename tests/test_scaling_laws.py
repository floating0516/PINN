...
@pytest.mark.parametrize(
    ("spec", "pgd_m", "distance_km", "expected"),
    [
...
def test_predict_mw_from_coefficients_matches_closed_form(spec, pgd_m, distance_km, expected):
    mw = predict_mw_from_coefficients(pgd_m=pgd_m, distance_km=distance_km, spec=spec)
...
def test_predict_mw_dispatch_accepts_lowercase_names(law_name):
    mw = predict_mw(law_name=law_name, pgd_m=0.05, distance_km=50.0)
...
def test_invalid_inputs_raise_value_error(pgd_m, distance_km):
    with pytest.raises(ValueError):
        predict_mw(law_name="melgar", pgd_m=pgd_m, distance_km=distance_km)


def test_unknown_scaling_law_raises_value_error():
    with pytest.raises(ValueError):
        predict_mw(law_name="unknown", pgd_m=0.05, distance_km=50.0)
